...
vignette: >
  %\VignetteIndexEntryHow to use the wqspt package}
...
$$
\sum_i=1}^N}(y_i-[\beta_0+\sum_j=1}^p}\beta_j x_ij}]})^2} + \lambda \sum_j=1}^p}\left\lvert\beta_j}\right\lvert}
$$
...
$$
\sum_d=1}^m}\sum_i=1}^N}(y_di}-[\beta_0d}+\sum_j=1}^p}\beta_dj} x_dij}}])^2}} + \lambda \sum_j=1}^p}\sqrt\sum_d=1}^m}\beta^2_dj}}}
$$
...
library(mice)
library(impgrplasso)
nh$Group <- as.factor(rep(LETTERS[1:2], times = c(12, 13)))
